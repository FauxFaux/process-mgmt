...
describe('calculating for output rates', function() {
    describe('basic, almost linear', function() {
        ...
        it('calculates material counts', function() {
            ...
            assert.strictEqual(result.materials.total_positive(data.items['a']).quantity, 0);
            assert.strictEqual(result.materials.total_negative(data.items['a']).quantity, -2);

            assert.strictEqual(result.materials.total_positive(data.items['b']).quantity, 0);
            assert.strictEqual(result.materials.total_negative(data.items['b']).quantity, -2);

            assert.strictEqual(result.materials.total_positive(data.items['c']).quantity, 2);
            assert.strictEqual(result.materials.total_negative(data.items['c']).quantity, -2);

            assert.strictEqual(result.materials.total_positive(data.items['d']).quantity, 1);
            assert.strictEqual(result.materials.total_negative(data.items['d']).quantity, -1);

            assert.strictEqual(result.materials.total_positive(data.items['e']).quantity, 1);
            assert.strictEqual(result.materials.total_negative(data.items['e']).quantity, -1);

            assert.strictEqual(result.materials.total_positive(data.items['f']).quantity, 1);
            assert.strictEqual(result.materials.total_negative(data.items['f']).quantity, 0);
        });
    });
});
