    ...
    describe('generates proxy processes from cycle', function () {
        ...
        describe('data with one self cycle; net consumer', function () {
            ...
            add_processes_to_data(data, {
                ...
                D: [
                    ['a', ['c', 2]],
                    ['c', 'd'],
                ], // 2 consumed, 1 produced; net 1 consumed.
                E: [['d'], ['e']],
                F: [['e'], ['f']],
            });
            it('proxies self cycles', function () {
            ...
            it('replaces cycle processes with a proxy', function () {
                ...
                chain.replace(
                    ['D'],
                    [new ProxyProcess([new RateProcess(data.processes['D'], data.factories['basic'])])],
                );
                assert.strictEqual(
                    true,
                    chain.processes.every(e => e.id !== 'D'),
                );
            ...
            it('expands proxy processes', function () {
                let chain = new RateChain(new ProcessChain(Object.values(data.processes)));
                chain = chain
                    .replace(['D'], [new ProxyProcess([new RateProcess(data.processes['D'], data.factories['basic'])])])
                    .update(new Stack(data.items['f'], 10), [])
                    .expand_proxies();
                assert.strictEqual(
                    true,
                    chain.processes.some(e => e.id === 'D'),
                );
        ...
        describe('data with two-element cycle; net producer', function () {
            ...
            it('replaces cycle processes with a proxy', function () {
                ...
                assert.strictEqual(
                    true,
                    chain.processes.every(e => e.id !== 'E'),
                );
                // console.log(chain.update(new Stack(data.items['f'], 10), []).to_graphviz());
            });
            it('expands proxy processes', function () {
                ...
                chain = chain.filter_for_output(
                    new Stack(data.items['f'], 10),
                    (item_id, options) => {
                ...
                assert.strictEqual(
                    true,
                    chain.processes.every(e => e.id !== 'C'),
                ); // proc 'C' should not be included as the proxy should be used instead.
                assert.strictEqual(
                    true,
                    chain.processes.some(e => e.id === 'D'),
                );
