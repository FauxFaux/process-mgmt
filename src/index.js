    ...
    fs.readFile(argv.config, 'utf8', (_err, str) => {
        ...
        import('./' + config.data + '/data.js').then(module => {
            ...
            let g = new ProcessChain(Object.values(data.processes))
                ...
                .accept(new EnableDisable(data, config.get_enabled(), config.get_disabled()))
                .accept(new StandardGraphRenderer())
                .join('\n');
            console.log(g);
        });
    });
                ...
                let g = new ProcessChain(Object.values(data.processes))
                    ...
                    .accept(
                        new RateVisitor(process => {
                            let f = config.get_factory_type(data, process.id, process.factory_group.id, () =>
                    ...
                    .accept(
                        new LinearAlgebra(config.get_requirements(data), config.get_imported(), config.get_exported()),
                    )
            ...
            let p = new ProcessChain(Object.values(data.processes))
                ...
                .accept(
                    new RateVisitor(process => {
                        ...
                        return f.modify(config.get_modifier_speed(process.id), config.get_modifier_output(process.id));
                    }),
                )
                .accept(
                    new RateCalculator(
        ...
        let material_output = p.materials
            ...
            .map(item => {
                ...
                return [
                    ...
                    'consume',
                    consume,
                    'producer count multiplier',
                    consume / produce,
                    'consumer count multiplier',
                    produce / consume,
                ];
            })
...
const command_update4 = function (argv) {
    ...
    import('./' + config.data + '/data.js').then(module => {
        ...
        p.process_counts = Object.entries(p.process_counts)
            .map(pair => {
    ...
    fs.readFile(argv.config, 'utf8', (_err, str) => {
        ...
        import('./' + config.data + '/data.js').then(module => {
            ...
            let g = new ProcessChain(Object.values(data.processes))
                ...
                .accept(new RateVisitor(factory_type => quickest_factory_for_factory_type(data, factory_type)))
                .accept(
                    new RateCalculator(
...
const process_to_pretty_string = function (p, data) {
    return (
        ...
        Object.values(data.factories)
            .filter(f => f.groups.map(fg => fg.name).includes(p.factory_group.name))
            .map(
                f =>
                    f.id +
                    ' (duration modifier: ' +
...
const command_what_produces = function (argv) {
    import('./' + argv.data + '/data.js').then(module => {
        ...
        console.log(options.map(p => process_to_pretty_string(p, data)).join('\n'));
    });
};
...
const argv = yargs(process.argv.slice(2))
    ...
    .command(
        'factory-graph',
        'generate a graph filtered to producing a particular item',
        yargs => {
            yargs
                .option('config', {
                    alias: 'c',
    ...
    .command(
        'factory-rate',
        'generate a graph filtered to producing a particular item; with factory counts.',
        yargs => {
            yargs
                .option('config', {
                    alias: 'c',
    ...
    .command(
        'factory-rate-loop',
        'generate a graph filtered to producing a particular item; with factory counts. Finding and removing loops',
        yargs => {
            yargs
                .option('config', {
                    alias: 'c',
    ...
    .command(
        'manual-rate',
        'generate a graph filtered to producing a particular item; with factory counts provided by the configuration',
        yargs => {
            yargs
                .option('config', {
                    alias: 'c',
